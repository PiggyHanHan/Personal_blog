...
// singleDollarTextMath 关闭单美元行内公式：正文里的货币符号（如 $3/百万tokens）
// 不会被误判成数学；行内公式统一走 \( \) → $$…$$ 的预转换。
const parser = unified()
  .use(remarkParse)
  .use(remarkGfm)
  .use(remarkMath, { singleDollarTextMath: false });

/** 数学定界符归一化：LaTeX 风格的 \( \) / \[ \] 转成 remark-math 认识的 $…$ / $$…$$。
 *  块级公式内嵌套的 \( \) 直接剥掉定界符（KaTeX 不支持嵌套定界，留着会污染输出）。
 *  代码段（``` 围栏 / 行内反引号）先摘出占位再转换——代码里出现的 \( \[ 是字面文本，
 *  不能被转成公式定界符（否则失衡的 $$ 会吞掉后文，渲染成大片 KaTeX 报错红字）。 */
function normalizeMathDelimiters(md: string): string {
  const stashed: string[] = [];
  const stash = (m: string) => `\u0000${stashed.push(m) - 1}\u0000`;
  const converted = md
  ...
  return converted.replace(/\u0000(\d+)\u0000/g, (_, i: string) => stashed[Number(i)]);
}

/** KaTeX → HTML（构建期渲染，SSG 输出；出错时显示红色原文而不是构建失败） */
...
export function markdownToBlocks(md: string): Block[] {
  const tree = parser.parse(normalizeMathDelimiters(md)) as Root;
  const blocks: Block[] = [];
...
function nodeToBlocks(node: RootContent): Block[] {
  switch (node.type) {
    ...
    case "math":
      // 块级公式（$$…$$ / \[…\]）：latex 留档，html 构建期渲染好
      return [
        {
          type: "math",
          latex: node.value,
          html: katexHtml(node.value, true),
        },
      ];
    case "table":
    ...
    case "inlineMath":
      return (
        <span
          ...
          dangerouslySetInnerHTML={{ __html: katexHtml(node.value, false) }}
        />
      );
    case "link": {
